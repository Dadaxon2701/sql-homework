...
SELECT
	YEAR(O.ORDERDATE) BYYEAR,
	AVG(O.Quantity) AVGAMOUNT
FROM Orders O
...
SELECT
	...
	CASE
		...
		WHEN P.PRICE>500 THEN 'HIGH'
		ELSE 'MID'
	END NN
FROM Products P
--Using City_Population table, use Pivot to show values of Year column in seperate columns ([2012], [2013]) and
...
SELECT 
	year,
	[Bektemir] AS Bektemir_Population,
	[Chilonzor] AS Chilonzor_Population,
	[Yakkasaroy] AS Yakkasaroy_Population
...
SELECT 
	[District_Name] = unpvt.Year_Column, 
	[Population] = unpvt.Population, 
	[Year] = p.Year
FROM Population_Each_Year p
UNPIVOT (
	Population FOR Year_Column IN ([2012], [2013])
) AS unpvt;
--Using Products and Sales tables, list product names and the number of times each has been sold. (Research for Joins)
SELECT
	P.ProductName,
	COUNT(S.SaleID) COUNTSALETIME
FROM Products P
JOIN Sales S
