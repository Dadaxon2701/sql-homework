...
-- 1. Barcha xodimlarning ismi va familiyasi
SELECT FirstName, LastName FROM Employees;

-- 2. HR bo‘limidagi xodimlar soni
SELECT COUNT(*) AS HR_Employees_Count
FROM Employees
WHERE DepartmentName = 'HR';

-- 3. Eng yuqori maosh oluvchi xodim
SELECT * FROM Employees
WHERE Salary = (SELECT MAX(Salary) FROM Employees);

-- 4. Yoshi 30 dan katta va IT bo‘limida ishlaydiganlar
SELECT * FROM Employees
WHERE Age > 30 AND DepartmentName = 'IT';

-- 5. Har bir bo‘limdagi o‘rtacha maosh
SELECT DepartmentName, AVG(Salary) AS Avg_Salary
FROM Employees
GROUP BY DepartmentName;

-- 6. Email manzili mavjud bo‘lgan xodimlar
SELECT * FROM Employees
WHERE Email IS NOT NULL;

-- 7. 2019-yildan keyin ishga qabul qilingan xodimlar
SELECT * FROM Employees
...
-- 1. Har bir kategoriyadagi mahsulotlar soni va ularning umumiy zaxirasi
SELECT 
    Category, 
...
-- 2. Elektronika kategoriyasidagi mahsulotlar va ularning narxi 100 dan yuqori bo‘lganlari
SELECT *
FROM Products_Discounted
WHERE Category = 'Electronics' AND Price > 100;

-- 3. Eng qimmat mahsulot (narxi bo‘yicha)
SELECT *
FROM Products_Discounted
WHERE Price = (SELECT MAX(Price) FROM Products_Discounted);

-- 4. Eng ko‘p zaxirada mavjud mahsulotlar
SELECT *
FROM Products_Discounted
WHERE StockQuantity = (SELECT MAX(StockQuantity) FROM Products_Discounted);

-- 5. Mahsulotlar narxlarining umumiy summasi va o‘rtacha narxi
SELECT 
    SUM(Price) AS TotalPrice,
    AVG(Price) AS AveragePrice
FROM Products_Discounted;

-- 6. Narxi 50 dan 200 gacha bo‘lgan mahsulotlar ro‘yxati
SELECT *
FROM Products_Discounted
...
-- 1. Har bir mahsulot bo‘yicha jami sotuv summasi
SELECT 
    p.ProductName,
    SUM(s.SaleAmount) AS TotalSales
FROM Sales s
JOIN Products_Discounted p ON s.ProductID = p.ProductID
GROUP BY p.ProductName
ORDER BY TotalSales DESC;

-- 2. Eng ko‘p sotilgan mahsulot (jami sotuv summasi bo‘yicha)
SELECT 
    p.ProductName,
    SUM(s.SaleAmount) AS TotalSales
FROM Sales s
...
-- 3. Har bir mijozning jami sarflagan summasi
SELECT 
    CustomerID,
...
-- 4. 2023-yil yanvar oyidagi kunlik jami sotuv summasi
SELECT 
    SaleDate,
...
-- 5. Mahsulot bo‘yicha o‘rtacha sotuv summasi
SELECT 
    p.ProductName,
