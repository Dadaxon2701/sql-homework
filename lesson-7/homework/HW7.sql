...
SELECT SUM(SaleAmount) TotalSalesAmountForProductID7
FROM Sales 
WHERE ProductID=7;
SELECT AVG(Age) AVGAGE
FROM Employees;
SELECT DepartmentName,
	   COUNT(*) AS HOWMANYEMPLOYEES
FROM Employees
...
SELECT
	ProductID,
	SUM(SaleAmount) AS TtlSales,
	AVG(SaleAmount) AS AvgSales
FROM Sales
GROUP BY ProductID;
SELECT COUNT(EmployeeID) AS DEPHR
FROM Employees
...
SELECT
	ProductID,
	SUM (TotalAmount) AS TTLSALES,
	MONTH(OrderDate) AS EACHMONTHS
FROM Orders
